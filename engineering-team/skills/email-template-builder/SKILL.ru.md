...
description: "Создавайте полноценные транзакционные почтовые системы: шаблоны электронной почты React, интеграция с поставщиками (повторная отправка, Postmark, SendGrid, AWS SES), сервер предварительного просмотра, поддержка i18n, темный режим, оптимизация спама, отслеживание аналитики. Используйте при добавлении транзакционной электронной почты в новый продукт, миграции между поставщиками электронной почты, рефакторинге устаревших шаблонов электронной почты для обеспечения доступности или добавлении интернационализации к существующим шаблонам."
---
...
- i18n/локализация с набранными ключами перевода
...
- Отслеживание открытия/клика с параметрами UTM
...
- **Нет гибкой коробки/сетки** — используйте `<Row>` и `<Column>` из react-email, а не из CSS-сетки
