---
name: "senior-secops"
description: "Старший инженер SecOps по скиллу безопасности приложений, управлению уязвимостями, проверке соответствия требованиям и методам безопасной разработки. Выполняет сканирование SAST/DAST, генерирует планы устранения CVE, проверяет уязвимости зависимостей, создает политики безопасности, применяет шаблоны безопасного кодирования и автоматизирует проверки соответствия требованиям SOC2, PCI-DSS, HIPAA и GDPR. Используется при проведении ревью или аудита безопасности, реагировании на CVE или инцидент с безопасностью, усилении инфраструктуры, внедрении аутентификации или управления секретами, выполнении подготовки к тестированию на проникновение, проверке уязвимости OWASP Top 10 или обеспечении контроля безопасности в пайплайнах CI/CD."
---
...
**Коды выхода:** `0` = нет критического/high выводы · `1` = результаты с высокой степенью серьезности · `2` = выводы о критической степени тяжести
...
**Коды выхода:** `0` = нет критического/high уязвимости · `1` = уязвимости высокой степени серьезности · `2` = уязвимости критической степени серьезности
...
### Чек-лист по безопасному кодированию (Secure Coding) { #secure-coding-checklist }
...
- **Требование 10/11**: Ведение журнала аудита, SAST/DAST/penetration тестирование
...
- МИД для человека/entity аутентификация (164.312(d))
...
Быстрая 15-минутная оценка — пройдите по каждой категории и отметьте проход/fail. Для тестирования на глубоком погружении обратитесь к скиллу **security-pen-testing**.
...
| A06 | Уязвимые компоненты | Бежать `vulnerability_assessor.py`; нулевой критический/high CVEs |
