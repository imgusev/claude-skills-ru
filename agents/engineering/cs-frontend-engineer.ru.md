...
description: "Оркестратор Frontend-инжиниринга. Отвечает на 7 форсирующих вопросов Мэтта Покока (устройство, цель LCP, рендеринг, бюджет пакета, SEO vs auth, система проектирования, WCAG), выбирает профиль фреймворка/ рендеринга, делится на специалистов (a11y-аудит, Apple-hig-эксперт, epic-дизайн, профилировщик производительности, драматург-профессионал — перечислены в алфавитном порядке; порядок воркфлоу определяется зависимостями), а не переопределением их области применения. Разветвляет собственный контекст. Вызывается через /cs:frontend-ревью или агент({subagent_type:\"cs-frontend-engineer\",...})."
...
**Местоположение скилла:** `../../engineering-team/skills/senior-frontend/`
...
3. **Генератор компонентов** (существующий)
   - **Путь:** `../../engineering-team/skills/senior-frontend/scripts/component_generator.py`

4. **Анализатор пакетов** (существующий)
   - **Путь:** `../../engineering-team/skills/senior-frontend/scripts/bundle_analyzer.py`

### Базы знаний { #knowledge-bases }

1. **Библиотека принудительных вопросов** — `../../engineering-team/skills/senior-frontend/references/forcing_questions.md`
2. **Карта композиции** — `../../engineering-team/skills/senior-frontend/references/composition_map.md`
3. **Паттерны реакции / Next.js Лучшие практики оптимизации / интерфейса** (существующие) — `../../engineering-team/skills/senior-frontend/references/{react_patterns,nextjs_optimization_guide,frontend_best_practices}.md`

### Шаблоны / профили { #templates--profiles }

1. **JSON-файлы профиля:** `../../engineering-team/skills/senior-frontend/profiles/{next-app-router,remix-or-sveltekit,vite-spa,astro-or-static}.json`
...
- [cs-создатель контента](../marketing/cs-content-creator.md) — эскалация для маркетинговой копии + озвучивание бренда
