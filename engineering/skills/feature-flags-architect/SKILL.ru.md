...
description: "Используйте при добавлении, удалении или аудите фич-флагов. Триггеры для \"добавить флаг\", \"отправить за флагом\", \"план раскатки\", \"kill switch\", \"устаревшие флаги\", \"задолженность по флагу\", \"LaunchDarkly\", \"GrowthBook\", \"Statsig\", \"Unleash\", \"Flipt\" или любой другой прогрессивной доставки вопрос. Поставляется сканер задолженности по флагам, планировщик раскатки и аудитор по отключению (все на stdlib Python), 4 ссылки на таксономию флагов + компромиссы с поставщиками + стратегии раскатки + жизненный цикл, а также слэш-команда /flag-cleanup."
context: fork
version: 2.9.0
...
Разные типы флагов имеют разную продолжительность жизни и право собственности. Неправильная классификация приводит к возникновению долгов.
...
| **Оперативный** | Автоматические выключатели, переключатели perf, kill Switch | месяцы–годы | Английский/SRE | Заменено на автоматическое масштабирование/feature выход на пенсию |
| **Разрешение** | Права на каждого пользователя/account/plan | годы (постоянные) | Продукт | План/role удаленный |
...
- `linear`: постоянная ставка в день. Дефолт по среднему риску.
...
|---|---|---|---|---|
| ** Запуск в темноте** | Предприятие, комплексный таргетинг, аудит/compliance | За МАУ, дорого | Высокий | Нет |
...
| **Статистика** | Рост/product команды, продвинутые эксперименты | Бесплатный уровень + per-MAU | Средний | Нет |
