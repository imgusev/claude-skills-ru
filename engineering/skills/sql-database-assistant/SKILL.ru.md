...
description: "Используйте, когда пользователь просит написать SQL-запросы, оптимизировать производительность базы данных, сгенерировать миграции, изучить схемы баз данных или поработать с ORM, такими как Prisma, Drizzle, TypeORM или SQLAlchemy."
---
...
- **Генерация миграции** — сценарии повышения/понижения, стратегии с нулевым временем простоя, планы отката
...
3. ** Определить фильтры** — сопоставить прилагательные/условия с предложениями WHERE
...
2. ** Определение самого дорогостоящего узла** — Последующее сканирование больших таблиц, вложенный цикл с высокими оценками строк
...
### Чек-лист рекомендаций по индексации Recommendation Recommendation { #index-recommendation-checklist }
...
2. **Фич—флаги** - приложение может переключаться между чтением старой/новой схемы
...
> Смотрите ссылки/orm_patterns.md для параллельных сравнений и воркфлоу миграции для каждого ORM.
...
1. ** Последовательный порядок блокировок** — всегда получайте блокировки в одном и том же порядке таблицы/строк
...
| Нет объединения подключений | Выпускные патрубки под нагрузкой | PgBouncer, ProxySQL или ORM-пул |
