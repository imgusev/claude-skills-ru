...
description: "Используйте при планировании, запуске или изучении результатов экспериментов по разработке хаоса. Триггеры для \"эксперимента с хаосом\", \"внедрения ошибки\", \"игрового дня\", \"теста на устойчивость\", \"радиуса поражения\", \"устойчивого состояния\", \"критериев прерывания\", \"Инструментария хаоса\", \"Сетки хаоса\", \"Лакмусовой бумажки\", \"Гремлина\", \"AWS FIS\" или любой преднамеренный сбой - вопрос о внедрении. Поставляется конструктор экспериментов, калькулятор радиуса взрыва и генератор посмертных данных (все на stdlib Python), 4 ссылки на принципы хаоса + дизайн эксперимента + таксономия атак + набор инструментов и слэш-команда /chaos-experiment. Компонуется с помощью feature-flags-architect (kill switch в качестве триггеров прерывания) и kubernetes-operator (общие цели хаоса)."
context: fork
version: 2.9.0
