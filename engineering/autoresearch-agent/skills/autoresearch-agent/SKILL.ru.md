...
description: "Автономный экспериментальный цикл, который оптимизирует любой файл по измеряемому показателю. Вдохновленный авторским исследованием Карпати. Агент редактирует целевой файл, запускает исправленную оценку, сохраняет улучшения (git commit), отбрасывает сбои (git reset) и выполняет цикл до бесконечности. Используйте, когда: пользователь хочет оптимизировать скорость кода, уменьшить размер пакета / изображения, повысить скорость прохождения теста, оптимизировать промпты, улучшить качество контента (заголовки, копирование, CTR) или запустить любой измеримый цикл улучшения. Требуется: целевой файл, команда оценки, которая выводит метрику, и git-репозиторий."
license: MIT
metadata:
  version: 2.0.0
...
2. Читать `program.md` о стратегии, ограничениях и о том, что вы можете / не можете изменить
...
- Если никаких улучшений в 20+ запусках не произошло: обновите program.md Стратегический раздел
...
- При ограничении контекста: следующий сеанс может возобновиться — results.tsv и git log сохраняются
...
- ** самосовершенствующийся - агент ** — улучшает собственную память / правила агента с течением времени. НЕ для структурированных циклов экспериментов.
