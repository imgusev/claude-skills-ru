---
name: "run"
description: "Одноразовая команда жизненного цикла, которая объединяет init → baseline → spawn → eval → merge в одном вызове. Используйте, когда пользователь запускает /hub:запускает или запрашивает выполнение полного сквозного соревнования AgentHub."
command: /hub:run
---
...
Если `--template` был предоставлен, используйте промпту отправки шаблона из `../agenthub/references/agent-templates.md` вместо промпта отправки по умолчанию. Передайте команду eval, метрику и базовую линию переменным шаблона.
