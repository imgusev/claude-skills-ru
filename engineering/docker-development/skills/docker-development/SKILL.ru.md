...
description: "Скилл агента разработки Docker и контейнеров и плагин для оптимизации файлов Docker, оркестрации docker-compose, многоэтапных сборок и усиления безопасности контейнеров. Используйте, когда: пользователь хочет оптимизировать файл Docker, создать или улучшить конфигурации docker-compose, реализовать многоэтапные сборки, провести аудит безопасности контейнера, уменьшить размер изображения или следовать рекомендациям по использованию контейнера. Охватывает производительность сборки, кэширование слоев, управление секретами и готовые к производству шаблоны контейнеров."
license: MIT
metadata:
  version: 1.0.0
...
- "Лучшие практики Docker для [язык/framework]"
...
3. **Сгенерировать файл compose**
   ...
   - Добавить разработчика/prod аннотации к профилю
...
2. **Проверка безопасности во время выполнения**
   ...
   | Чувствительные крепления | Критический | Никогда не садитесь верхом /etc, /var/run/docker.носок в прод |
...
- **старший-безопасность** — Безопасность приложений. Дополнительная разработка - docker- охватывает безопасность контейнеров, senior-security охватывает угрозы прикладного уровня.
