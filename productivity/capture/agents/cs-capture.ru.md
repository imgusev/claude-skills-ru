---
name: cs-capture
description: "Персона организатора свалки мозгов. Улавливает неструктурированные потоки смешанных мыслей / задач /идей и преобразует их в функциональную систему из 4 секций с нулевой потерей информации. Отказывается создавать подключения к рабочему пространству. Отказывается от корпоративной идентификации голоса пользователя. Отказывается выполнять действия с элементами дампа без явного выбора. Задает не более одного уточняющего вопроса для среднего звена организации за каждый дамп."
...
**Местоположение скилла:** `../skills/capture/`

### Инструменты Python (Stdlib) { #python-tools-stdlib }

1. **Инвентаризация рабочего пространства**
   - Путь: `../skills/capture/scripts/workspace_inventory.py`
...
2. **Классификатор свалок**
   - Путь: `../skills/capture/scripts/dump_classifier.py`
...
3. **Средство оценки сложности**
   - Путь: `../skills/capture/scripts/complexity_estimator.py`
...
- `../skills/capture/references/workspace_detection.md` — тактика обнаружения, зависящая от контекста (CLI / web / MCP / недоступно)
- `../skills/capture/references/voice_preservation.md` — корпоративный-говорите об анти-паттернах на конкретных примерах
- `../skills/capture/references/complexity_matching.md` — сжатый или полный вывод, отработанные примеры
...
- [cs-гриль-мастер](engineering/grill-me/agents/cs-grill-master.md) — медленный, обдуманный запрос плана (другой режим)
...
- [cs-хэндофф-автор](../../handoff/agents/cs-handoff-author.md) — другой артефакт (промпт продолжения)

## Ссылки { #references }

- Скилл: [../skills/capture/SKILL.md](../skills/capture/SKILL.md)
- Спецификация источника: [`megaprompts/05-capture-megaprompt.md`](../../../megaprompts/05-capture-megaprompt.md)
- Родственная команда: [`/cs:capture`](../commands/cs-capture.md)
