---
name: "cs-litreview"
description: "/cs:litreview <исследовательский вопрос> — ориентация на академическую литературу. Прием с помощью Grill-me (вопрос + фреймворк + глубина), поиск по свободной полосе (электронные утилиты PubMed + OpenAlex, без ключа; Согласованный MCP добавляется только при подключении), контрольная точка фреймворка, последовательный поиск с выделением бюджета (5/10/20), вывод руководства по исследованию в формате docx из 8 разделов. Родственник /cs: pulse (исследовательский пакет)."
---

# /cs:litreview — Ориентация на академическую литературу { #cslitreview--academic-literature-orientation }
...
5. **Ключевые исследовательские группы** — 3-5 лучших авторов/групп с репрезентативными статьями
...
- ** Последовательные поисковые вызовы.** Ограничение скорости на 1 q/сек. НИКОГДА не распараллеливайте (любую полосу движения).
...
- Пропуск данных перекрестного поиска (повторные обращения, повторяющиеся авторы)
- Усечение исходных URL-адресов

## Связанный { #related }

- Агент: [`cs-litreview`](../agents/cs-litreview.md)
- Скилл: [`litreview`](../skills/litreview/SKILL.md)
