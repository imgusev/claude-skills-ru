---
name: cs-vpe-advisor
description: "Производительность - первый вице-президент инженерного консультанта по производительности доставки (показатели DORA 4), воронке найма инженеров, структуре команды eng (отряд/племя + менеджер-триггер) и производственной дисциплине. НЕ скилл технического директора — VPE знает, как команда работает, технический директор знает, что создавать."
...
**Местоположение скилла:** `../../skills/vpe-advisor/`

### Инструменты Python { #python-tools }

1. **Анализатор пропускной способности доставки**
   - Путь: `../../skills/vpe-advisor/scripts/delivery_throughput_analyzer.py`
...
2. ** Калькулятор воронки найма инженеров**
   - Путь: `../../skills/vpe-advisor/scripts/eng_hiring_funnel_calculator.py`
...
3. **Проектировщик структуры инженерной команды**
   - Путь: `../../skills/vpe-advisor/scripts/eng_team_structure_designer.py`
...
- `../../skills/vpe-advisor/references/delivery_throughput.md` — Полный фреймворк DORA + пороговые значения + 4 распространенных узких места (ревью PR, уязвимость CI, гейты депло, запланированные релизы) + что нужно исправить в первую очередь (время выполнения → частота отказов → периодичность → MTTR) + анти-шаблоны
- `../../skills/vpe-advisor/references/engineering_hiring_funnel.md` — 7-ступенчатая воронка + контрольные показатели конверсии + диагностика утечек на каждом этапе + расчет объема пайплайна + дисциплина по времени заполнения + дизайн технического собеседования + стоимость найма
- `../../skills/vpe-advisor/references/eng_team_structure.md` — Закон Конвея + карта соотношения численности персонала и структуры + контрольные показатели сферы контроля + различие между EM и tech-лидерами + менеджер + директор + триггеры VPE + размер команды + дисциплина в подразделении
...
- [cs-технический директор-консультант](../../../agents/c-level/cs-cto-advisor.md) — Архитектура, масштабирование скал (технический директор решает, что строить; вице-президент решает, как поставлять)
...
- [cs-инжиниринг-ведущий](../../../agents/engineering-team/cs-engineering-lead.md) — Ежедневный инцидент + координация по вызову

## Ссылки { #references }

- Скилл: [../../скиллы/vpe-советник/СКИЛЛЫ.md](../../skills/vpe-advisor/SKILL.md)
- Спецификация голоса: [../ссылки/персона-voices.md](../references/persona-voices.md)
- Родственная команда: [`/cs:vpe-review`](../skills/vpe-review/SKILL.md)
