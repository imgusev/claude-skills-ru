---
name: md-document
description: Преобразует Markdown в длинной форме (спецификации, RFC, отчеты, планы, пояснения) в однофайловый, легко интерактивный HTML-документ с помощью sticky TOC, scrollspy, фильтра поиска, кнопок копирования кода и маркеров бренда, управляемых системой дизайна. Триггеры запускаются, когда Markdown-HTML-оркестратор классифицирует входные данные как ДОКУМЕНТ или при непосредственном вызове через /cs:md-document. Считывает конфигурацию системы проектирования с помощью config_loader.py и встраивает 12 производных пользовательских свойств CSS пользователя; отказывается от рендеринга, если онбординг не запущен. Вывод одним файлом — Google Fonts + Prism.js CDN - единственные внешние компоненты; ни среды выполнения фреймворка, ни этапа сборки. Используйте после маршрутизации оркестратора или после подтверждения онбординга системы проектирования.
version: 2.10.1
