---
description: "Преобразуйте PR-запись Markdown или ревью кода (с блоками ``diff и > [!BLOCKER]/[!MAJOR]/[!MINOR]/[!NIT] выносками серьезности) в однофайловый HTML-ревью с 2 столбцами. Top jump-навигатор отображает все находки в разбивке по степени серьезности. Разница слева, карточки с аннотациями с пометками серьезности справа, обязательный нижний колонтитул с именем рецензента. Отказывает без --reviewer (в ревью кода должен быть указан человек) или если нет различий."
argument-hint: "<path to markdown> --reviewer \"<name>\" [--title \"<PR title>\"] [--severity-convention \"<a,b,c,d>\"]"
---

# /cs:md-ревью — ревью кода- Markdown → HTML с 2 столбцами { #csmd-review--code-review-markdown--2-column-html }

Преобразуйте ревью Markdown в **$ARGUMENTS** в однофайловый HTML-ревью с 2 столбцами.
...
4. **Никаких отличий нет** → отказаться, направить в `md-document` вместо этого.
...
- **2-столбчатые строки** — унифицированные различия слева (номера строк с обеих сторон, знаки +/−, добавление/удаление оттенков bg из маркеров системы дизайна), карточки с аннотациями справа
