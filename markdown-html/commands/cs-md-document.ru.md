---
description: "Преобразуйте Markdown в расширенной форме (спецификации, RFC, отчеты, планы, пояснения) в однофайловый интерактивный HTML-документ. Запускает пайплайн md-документа (синтаксический анализатор → средство визуализации → инжектор) с токенами пользовательской системы проектирования. Sticky TOC, фильтр поиска, кнопки копирования кода, scrollspy, палитра WCAG-AA. Отказывается вводить < 100 строк (Shihipar) или незавершенный онбординг. Вывод представляет собой один HTML-файл (Google Fonts + Prism.js CDN как единственные внешние устройства)."
