...
**Ценовая дисциплина:** постройте модель затрат с помощью таблицы—заполнителя - столбцов `model | $/1M tokens (verify) | dims | as-of date` — и попросите пользователя ввести реальные цифры. То же самое для векторных баз данных (Сосновая шишка/плетение/Qdrant/цветность/pgvector): критерии выбора (управляемый или автономный хостинг, масштабирование, фильтрация, существующий Postgres) являются надежными; показатели в долларах - нет.
...
3. Каждая рекомендуемая модель/price в поставляемом документе указано "проверить текущие цены/model примечание "доступность" с указанием текущей даты.
...
- `references/rag_evaluation_framework.md` — метрические определения (достоверность, релевантность, точность/recall/NDCG)
