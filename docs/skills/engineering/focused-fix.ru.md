...
Активируется, когда пользователь просит исправить, отладить или создать определенную функцию/module/area работа. Ключевые триггеры:
- "заставь X работать"
- "исправьте функцию Y"
- "модуль Z сломан"
...
1. Спросите пользователя: "Какая функция/folder должен ли я сосредоточиться на этом?", если еще не ясно
2. Определите основную папку/files для этой функции
...
2. Для каждого потребителя:
   ...
   - Проверьте, используют ли они правильный API/interface
...
**КАЧЕСТВО КОДА:**
- [ ] Каждый импорт преобразуется в реальный файл/export
...
- Отслеживание данных/control пройдите в обратном направлении, чтобы убедиться — не доверяйте симптомам на поверхностном уровне
...
| МЕД | Внутренний модуль с тестами / общая утилита / конфигурация с влиянием на время выполнения / внутренние вызывающие устройства измененных функций |
...
1. Запустите ВСЕ тесты в папке функций — каждый из них должен пройти
