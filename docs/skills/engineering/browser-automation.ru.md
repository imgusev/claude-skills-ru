...
- **Async-first** — Python async/ожидание для очистки с высокой пропускной способностью
...
### 2. Заполнение форм и многоступенчатые воркфлоу { #2-form-filling--multi-step-workflows }
...
- ** Списки в массивы** — Сопоставление повторяющихся элементов карточки с использованием карты выбора полей (поддерживает `::attr()` для атрибутов)
- **Вложенные/потоковые данные** — Рекурсивное извлечение комментариев с ответами, деревьев категорий
...
- **Сохранение/восстановление файлов cookie:** `context.cookies()` и `context.add_cookies()`
...
Видишь [anti_detection_patterns.md](https://github.com/imgusev/claude-skills-ru/tree/main/engineering/skills/browser-automation/references/anti_detection_patterns.md) для полного стелс-стека: усиление свойств навигатора, уклонение от отпечатков пальцев WebGL / canvas, поведенческое моделирование (движение мыши, скорость набора текста, шаблоны прокрутки), стратегии поворота прокси-серверов и самотестирование URL-адресов обнаружения.
...
### Никаких ограничений по ставкам { #no-rate-limiting }
**Плохо:** Забивание сайта со скоростью 100 запросов в секунду.
