---
title: "Архитектор SLO { #slo-architect } — Агентский скилл для Codex и OpenClaw"
description: "Используйте при определении, ревью или управлении бюджетами SLO/SLIS/ошибок. Триггеры для 'определения SLO', 'каким должен быть наш SLO', 'бюджет. Агентский скилл для Claude Code, Codex CLI, Gemini CLI, OpenClaw."
---
...
<span class="meta-badge">:material-github: <a href="https://github.com/imgusev/claude-skills-ru/tree/main/engineering/skills/slo-architect/SKILL.md">Источник</a></span>
</div>
...
- Активное реагирование на инцидент → использовать `incident-response`
...
- `cpu_as_sli`: Центральный процессор/memory используется в качестве прокси-сервера пользовательского интерфейса (неверный сигнал)
...
- 100% SLOs проходят `slo_review.py` с 0 ошибочными выводами
- У каждого SLO есть задокументированный владелец, бюджет ошибок, предупреждения о частоте выгорания и политика
- Предупреждения о скорости горения срабатывают ≤2 раза/month за каждый попавший SLO (сигнал, а не шум)
