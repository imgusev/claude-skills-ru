---
title: "Архитектор миграции { #migration-architect } — Агентский скилл для Codex и OpenClaw"
description: "Планирование миграции с нулевым временем простоя, проверка совместимости и выработка стратегии отката. Инструменты для миграции систем, баз данных и. Агентский скилл для Claude Code, Codex CLI, Gemini CLI, OpenClaw."
---
...
<span class="meta-badge">:material-github: <a href="https://github.com/imgusev/claude-skills-ru/tree/main/engineering/skills/migration-architect/SKILL.md">Источник</a></span>
</div>
...
**Гейт:** миграция не будет одобрена до тех пор, пока (a) `compatibility_checker` выходы 0 (`overall_compatibility: compatible`) или каждое нарушение/potentially-breaking элемент явно принят владельцем в письменной форме, и (б) для каждого этапа плана существует рансбук отката. Повторно запустите обе проверки после любой ревизии схемы.
...
1. **Схема расширения контракта**
   - **Развернуть:** Добавить новые столбцы/tables наряду с существующей схемой
   ...
   - **Контракт:** Удалить старые столбцы/tables после проверки
...
1. **Перехват запросов:** Маршрутизация трафика через прокси-сервер/gateway
...
3. **Производственная миграция**
   ...
   - Внедрять межоблачные сети во время перехода
...
1. **Сине-зеленое развертывание**
   ...
   - Поддерживайте параллельную инфраструктуру во время периода миграции
