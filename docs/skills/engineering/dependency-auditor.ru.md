...
Автономный детерминированный аудит зависимостей в более чем 8 экосистемах пакетов. Эти три сценария сопоставляют шаблоны с манифестами/lockfiles — они ** не** вызывают API-интерфейсы live advisory; сопоставляют свои выводы с `npm audit` / `pip-audit` / `cargo audit` для текущего покрытия CVE.
...
Потребляйте выходные данные: `scan.json` результаты определяют, какие пакеты следует закрепить/patch сейчас; `licenses.json` конфликты передаются пользователю в виде списка юридических рисков; `plan.json` заказывает обновления по степени риска с пометками об откате. `--quick-scan` пропускает переходные значения deps; `--security-only` ограничивает план исправлениями безопасности.
...
- **Авторское лево (сильное)**: GPL v2/v3, AGPL v3 — указывает на риск загрязнения в разрешительных проектах
...
1. ** Уделяйте приоритетное внимание безопасности**: устраняйте высокие/critical выводы делаются немедленно; соответствие лицензии предшествует функциональности.
2. ** Постепенные обновления **: поэтапные обновления с тщательным тестированием; фич-флаги для опасных ударов.
...
Видишь [README.md](https://github.com/imgusev/claude-skills-ru/tree/main/engineering/skills/dependency-auditor/README.md) для подробного использования и `references/` за уязвимость/license базы знаний.
