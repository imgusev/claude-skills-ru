---
title: "Старший серверный инженер { #senior-backend-engineer } — Агентский скилл и плагин Codex"
description: "Разрабатывает и внедряет серверные системы, включая REST API, микросервисы, архитектуры баз данных, потоки аутентификации и усиление безопасности. Агентский скилл для Claude Code, Codex CLI, Gemini CLI, OpenClaw."
---
...
<span class="meta-badge">:material-github: <a href="https://github.com/imgusev/claude-skills-ru/tree/main/engineering-team/skills/senior-backend/SKILL.md">Источник</a></span>
</div>
...
1. **Читать/write коэффициент + годовой p99 QPS** — управляет базой данных, кэшем, очередью и выбором разделов. Клеппманн, *DDIA* (2017).
...
Тот `scripts/backend_decision_engine.py` инструмент кодирует эти проверки: он отказывается рекомендовать профиль без прочтения/write соотношение + QPS + срок аренды + чувствительность к данным + предпочтение шаблона.
...
3. **Прямой вызов инструмента:** `python scripts/backend_decision_engine.py ...` — детерминированный профиль совпадает, когда известны входные данные.
