...
Детерминированный выбор профиля. Учитывая четыре допущения (размер команды, частота кадров, ориентация на пользователя, бюджет) плюс дополнительный трафик/sensitivity вводит данные, ранжирует четыре встроенных профиля и возвращает согласованный профиль с уровнем SLO и именованной цепочкой утверждающих. Отказывается рекомендовать профиль без четырех необходимых входных данных.
...
Возвращает: совпадающее имя профиля, оценка, совпадающий/violated ограничения, рекомендации по стеку, анти-рекомендации, уровень SLO, цепочка именованных утверждающих и ссылки на канон.
...
2. **Бэкенд с большим количеством API?** → Отдельный FastAPI или NestJS
...
3. ** Ориентированный на пользователя, внутренний и маркетинговый сайт ** - управляет выбором стека и a11y/perf бюджет.
4. ** Ежемесячный лимит бюджета на облако + SaaS ** — обеспечивает разделение между сборкой и управляемым сервисом.
...
Если что—либо из этих трех не указано, рекомендация неполна - вернитесь к Q7 библиотеки принудительных вопросов.
...
|---|---|---|---|
| `saas-startup` | < 10 англ., ориентированный на клиента, ежедневно + частота | 8 ТЫСЯЧ долларов/mo | Модульный монолит на Next.js + Postgres |
| `enterprise-scale` | 50+ eng, регулируемый, per-PR с гейтами | 250 ТЫСЯЧ долларов/mo | Сервисы, ограниченные доменом + команда платформы |
| `internal-tool` | ≤ 5 eng, с автоматической стенкой, < 100 DAU | $500/mo | Сначала переустановите; уменьшите пользовательский стек, если принудительно |
| `marketing-site` | Зависимая от SEO, почти нулевая запись | $200/mo | Статический-первый (Astro / 11ty / Следующий-статический) |
