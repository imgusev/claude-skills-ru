---
title: "Старший разработчик { #senior-devops } — Агентский скилл и плагин Codex"
description: "Всесторонний скилл DevOps для CI/CD, автоматизации инфраструктуры, контейнеризации и облачных платформ (AWS, GCP, Azure). Включает в себя настройку. Агентский скилл для Claude Code, Codex CLI, Gemini CLI, OpenClaw."
---
...
<span class="meta-badge">:material-github: <a href="https://github.com/imgusev/claude-skills-ru/tree/main/engineering-team/skills/senior-devops/SKILL.md">Источник</a></span>
</div>
...
Генерирует манифесты развертывания Kubernetes и упорядоченные рансбуки kubectl для blue/green или переходящие стратегии с гейтами проверки работоспособности перед переключением трафика и откатом рансбуков. Инструмент записывает манифесты и печатает команды — он никогда не применяет их к самому кластеру, поэтому каждое изменение проходит ревью человека.

**Пример — Kubernetes blue/green развертывание (элементы, специфичные для синего слота):**
```yaml
...
- Ссылка на образец: `references/cicd_pipeline_guide.md` — подробные шаблоны CI/CD, лучшие практики, анти-шаблоны
...
- **Мультиоблачный** - требуется, если это требуется в соответствии с требованиями законодательства/data резидентство, приобретение компаний в разных облаках или потребность в лучших в своем классе сервисах от разных поставщиков (например, AWS для compute + GCP для ML)
- **Гибрид** — on-prem + облако; используется, когда регулируемые рабочие нагрузки должны оставаться включенными во время пакетной обработки/non-sensitive рабочие нагрузки выполняются в облаке
...
## IaC, не зависящий от облачных вычислений { #cloud-agnostic-iac }
