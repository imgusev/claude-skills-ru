---
title: "Почтовый ящик-агент настройки { #inbox-setup-agent } — ИИ-агент для Claude Code и Codex"
description: "Одноразовая рассылка по электронной почте- сортировка онбординга персоны. Проводит интерактивное интервью из 8 разделов (~ 25-31 вопрос grill-me). Агентский оркестратор для Claude Code, Codex, Gemini CLI."
---
...
<span class="meta-badge">:material-github: <a href="https://github.com/imgusev/claude-skills-ru/tree/main/productivity/email/agents/cs-inbox-setup.md">Источник</a></span>
</div>
...
>
> Запустите скилл **Сортировка входящих сообщений**, чтобы обработать свой почтовый ящик. Первые запуски требуют контроля — система учится на ваших правках и переопределениях. Повторно запускайте настройку в любое время, когда меняется бизнес / цены /приоритеты."
...
3. ** Принудительный формат, где это возможно.** Множественный выбор > открытый. S2.Q1 ("соответствует ли это: да/в основном/нет"), а не "что вы думаете?"
...
8. ** Повторный запуск в безопасном режиме.** Промпт замены/слияния/пропуска для существующих файлов для каждого файла.

## Интеграция в скиллы { #skill-integration }

**Местоположение скилла:** [`skills/inbox-setup`](https://github.com/imgusev/claude-skills-ru/tree/main/productivity/email/skills/inbox-setup)

### Инструменты Python (Stdlib) { #python-tools-stdlib }

1. **Валидатор KB**
   - Путь: [`scripts/kb_validator.py`](https://github.com/imgusev/claude-skills-ru/tree/main/productivity/email/skills/inbox-setup/scripts/kb_validator.py)
...
2. ** Отслеживание прогресса раздела**
   - Путь: [`scripts/section_progress_tracker.py`](https://github.com/imgusev/claude-skills-ru/tree/main/productivity/email/skills/inbox-setup/scripts/section_progress_tracker.py)
...
3. **Анализатор голосовых выборок**
   - Путь: [`scripts/voice_sample_analyzer.py`](https://github.com/imgusev/claude-skills-ru/tree/main/productivity/email/skills/inbox-setup/scripts/voice_sample_analyzer.py)
   - Использование: `python voice_sample_analyzer.py --samples-file /tmp/samples.txt`
   - Извлекает речевые паттерны из вставленных образцов отправленных электронных писем: вступительные фразы, подписи, длина предложения, типы предложений, случайные / формальные маркеры.

### Базы знаний { #knowledge-bases }

- [`references/kb_file_contract.md`](https://github.com/imgusev/claude-skills-ru/tree/main/productivity/email/skills/inbox-setup/references/kb_file_contract.md) — канонический контракт на 7 файлов (перспектива записи)
- [`references/grill_me_section_walk.md`](https://github.com/imgusev/claude-skills-ru/tree/main/productivity/email/skills/inbox-setup/references/grill_me_section_walk.md) — дисциплина из 8 разделов + логика пропуска + фиксация для каждого раздела
- [`references/voice_calibration.md`](https://github.com/imgusev/claude-skills-ru/tree/main/productivity/email/skills/inbox-setup/references/voice_calibration.md) — теория извлечения голоса на основе сэмплов + анти-паттерны
...
- [cs-захват](https://github.com/imgusev/claude-skills-ru/tree/main/productivity/capture/agents/cs-capture.md) — органайзер для сброса мозгов (другой режим)

## Ссылки { #references }

- Скилл: [../скиллы/входящие-настройка/СКИЛЛЫ.md](https://github.com/imgusev/claude-skills-ru/tree/main/productivity/email/skills/inbox-setup/SKILL.md)
...
- Родственная команда: [`/cs:inbox-setup`](https://github.com/imgusev/claude-skills-ru/tree/main/productivity/email/commands/cs-inbox-setup.md)
