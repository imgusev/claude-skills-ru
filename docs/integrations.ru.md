...
-   :material-medical-bag:{ .lg .middle } **Агент Hermes**
...
|--------|--------|--------------|
| Передняя панель YAML | Материал для фасада, предназначенный для конкретного инструмента | Названия/значения полей адаптированы для каждого инструмента |
...
В Windsurf используется прогрессивное раскрытие информации — по умолчанию отображаются только название и описание скилла. Полный `SKILL.md` контент загружается только тогда, когда Windowserf решает, что скилл соответствует вашему запросу, сохраняя при этом ограниченность контекстного окна.
...
После установки скиллы доступны через стандартное приложение Vibe discovery (в соответствии с [Документы о агентах Vibe и скиллах](https://docs.mistral.ai/mistral-vibe/agents-skills)):
...
| Скрипты на Python (`scripts/`) | ✅ | Все только для stdlib; инструмент оболочки Vibe запускает их |
| Ссылки / шаблоны / ресурсы | ✅ | То же соглашение о каталогах |
| Слэш-команды (`/<name>`) | ✅ | Автоматически обнаруживается из SKILL.md |
...
??? question "Vibe doesn't see the synced skills"
    ...
    ```
    Если символические ссылки нарушены (исходное хранилище было перемещено), запустите повторно `python scripts/sync-vibe-skills.py --verbose`.
