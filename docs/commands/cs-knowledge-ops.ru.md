...
2. **`runbook_validator.py`** — Проверка полноты Рансбука: у каждого шага есть владелец, ожидаемая продолжительность, наблюдаемый успех/failure сигнал, путь отката. Указывает на неоднозначность ("убедитесь, что служба запущена" → "какова команда проверки?").
