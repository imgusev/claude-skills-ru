...
description: "Категоризация расходов + рационализация поставщиков + анализ цикла закупок. НЕ оценка эффективности работы поставщика (родственное управление. Слэш-команда для Claude Code, Codex CLI, Gemini CLI."
---
...
2. **`purchasing_cycle_analyzer.py`** — Время до получения заказа, время до оплаты, одобрение-количество переходов по категориям. Помечает категории со временем цикла > 2× медиана.
