...
1. Коэффициент чтения/записи + годичный p99 QPS
2. Модель аренды (одиночная / общая / изолированная мультитенантная)
3. Запрос/ответ синхронизации в сравнении с асинхронным (очередь) и управляемым событиями
...
5. **Разделяться на специалистов в порядке зависимости:**
...
- ❌ Рекомендуется использовать базу данных без соотношения QPS + чтение/запись (вопрос 1 остался без ответа).
